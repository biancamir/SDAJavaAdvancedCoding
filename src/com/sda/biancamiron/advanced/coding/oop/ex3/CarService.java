...
public class CarService {
    ...
    public Optional<Car> getMostCheapestCarWithStream() {
        return cars.stream()
                .min(((o1, o2) -> Double.compare(o1.getPrice(), o2.getYearOfManufacturer())));
    }
                    ...
                    switch (operator) {
                        case ">":
                            return car.getManufacturers().stream()
                                    .anyMatch(manufacturer -> manufacturer.getYearOfEstablishment() > year); //verifica dc ai cel putin o conditie valida ( boolean) ai anymatch sau allmatch
                        case "<":
                            return car.getManufacturers().stream()
